...
export default async function EditProjectPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_right,rgba(103,80,164,0.14),transparent_28%),var(--sys-color-role-surface)]">
      ...
      <main className="mx-auto flex max-w-6xl flex-col gap-4 px-4 py-5 md:px-6 md:py-6">
        <nav className="flex flex-wrap items-center gap-1 md-typescale-label-large" aria-label="Breadcrumb">
          ...
          <Link href={`/projects/${id}`} className="text-[var(--sys-color-role-primary)] min-h-[44px] inline-flex max-w-[220px] items-center truncate">
            {project.client_name}
          </Link>
          <span className="text-[var(--sys-color-role-on-surface-variant)]">/</span>
          <span className="text-[var(--sys-color-role-on-surface-variant)] min-h-[44px] inline-flex items-center">Редактирование</span>
        </nav>
